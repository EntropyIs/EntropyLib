...
namespace Entropy
{
	namespace Math
	{
		Vec4 __declspec(dllexport) operator* (const Matrix4& left, const Vec4& right);

		//Transformations
		Matrix4 __declspec(dllexport) ScaleMatrix4(const Vec4& scaleVector);
		...
		Matrix4 __declspec(dllexport) TranslationMatrix4(const Vec4& translationVevtor);
		...
		Matrix4 __declspec(dllexport) RotationAboutXYZMatrix4(const Vec4& rotationVector);
		...
		Matrix4 __declspec(dllexport) RotationAboutAxisMatrix4(const Vec4& axis, const float angle);
		...
		Matrix4 __declspec(dllexport) LookAt(Vec3 cameraPos, Vec3 cameraTarget, Vec3 cameraUp);
	}
}
